...
def LoadTicker(tickerSymb, dateBegin, dateEnd):
    ...
    while (begin <= end): 
        ...
        name = f"./{tickerSymb}/{TickerFilename2str(tickerSymb, batch_start, batch_end)}"
        if not (data is None):
            data.to_csv(name, index=False)
            print("loaded", batch_start, batch_end)
        else:
...
dataBegin = "2010-01-01"
dataEnd   = "2025-10-10"

LoadTicker("SBER", dataBegin, dataEnd)