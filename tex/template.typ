...
*Применение LSTM в финансовых временных рядах.* Экспериментальные исследования показывают, что LSTM-модели превосходят традиционные методы временных рядов, такие как ARIMA, с существенным отрывом при прогнозировании финансовых данных @siami2019comparative. В работе @siami2019comparative проведено сравнительное исследование ARIMA, одностороннего LSTM и двунаправленного LSTM (BiLSTM) на данных акций IBM. Результаты демонстрируют, что глубокие нейронные сети с памятью, такие как LSTM, превосходят регрессионные подходы благодаря способности моделировать нелинейные зависимости и запоминать длинные последовательности входных данных через механизм ворот.

В задачах прогнозирования цен акций LSTM-сети применяются для предсказания будущих значений на основе исторических данных. Модель обучается на последовательностях фиксированной длины, где каждое окно содержит признаки за определённый период, и предсказывает значение цены на следующий временной шаг. Эксперименты @siami2019comparative показывают, что LSTM-модели обрабатывают данные в одном направлении (слева направо), деля их на батчи размером 41-42 элемента, и достигают равновесия быстрее, чем двунаправленные варианты.

*Двунаправленный LSTM (BiLSTM) в финансах.* Архитектура BiLSTM расширяет возможности обычного LSTM за счёт дополнительного обучения данных в обратном направлении (справа налево), что позволяет модели учитывать как прошлые, так и будущие контексты при обработке каждого элемента последовательности @siami2019comparative. Экспериментальные результаты @siami2019comparative демонстрируют, что BiLSTM превосходит односторонний LSTM на $37.78\%$ по снижению ошибок прогнозирования при работе с финансовыми временными рядами. Это улучшение достигается за счёт того, что модель обучается на данных дважды: сначала в прямом направлении (от прошлого к будущему), затем в обратном (от будущего к прошлому), что позволяет извлекать дополнительные паттерны из данных.

Интересной особенностью BiLSTM, выявленной в @siami2019comparative, является то, что модель делит те же данные на большее количество батчей (71-75 против 41-42 у обычного LSTM), используя меньшие размеры батчей. Это связано с тем, что BiLSTM должна обрабатывать данные в обоих направлениях, и длина обучающих данных, обрабатываемых каждым батчем, составляет примерно половину от длины батча в обычном LSTM. Кроме того, BiLSTM достигает равновесия медленнее, чем односторонний LSTM, что указывает на способность модели извлекать дополнительные признаки из данных, которые не могут быть обнаружены при одностороннем обучении.
...
*Применение GRU в прогнозировании финансовой волатильности.* В работе @garchgru2024 предложена гибридная модель, объединяющая GARCH (Generalized Autoregressive Conditional Heteroskedasticity) и GRU для прогнозирования волатильности финансовых временных рядов. GARCH-модели традиционно используются для моделирования условной гетероскедастичности (изменяющейся во времени волатильности) в финансовых данных, но имеют ограничения при работе с нелинейными зависимостями. Интеграция GRU с GARCH позволяет комбинировать преимущества статистических моделей волатильности с способностью глубоких нейронных сетей улавливать сложные нелинейные паттерны. В такой гибридной архитектуре GARCH-компонент обеспечивает структурированное моделирование условной дисперсии, а GRU-компонент извлекает дополнительные нелинейные зависимости из остатков и других признаков, что улучшает точность прогнозирования волатильности по сравнению с использованием только GARCH или только GRU.

*Рекуррентные сети с механизмом внимания для долгосрочного прогнозирования.* Традиционные RNN, LSTM и GRU эффективны для одношагового и краткосрочного прогнозирования, но сталкиваются с трудностями при долгосрочном прогнозировании многомерных финансовых временных рядов @liu2019dstp. Ключевая проблема долгосрочного прогнозирования заключается в необходимости одновременного захвата трёх типов зависимостей: пространственных корреляций между различными атрибутами в один момент времени, пространственно-временных отношений между различными атрибутами в разные моменты времени, и временных зависимостей между различными рядами @liu2019dstp.

Механизм внимания в RNN позволяет модели динамически определять веса различных входных признаков и временных шагов, что улучшает способность модели извлекать релевантную информацию из многомерных временных рядов. В работе @liu2019dstp предложена архитектура DSTP-RNN (Dual-Stage Two-Phase RNN), вдохновлённая моделью человеческого внимания DSTP (Dual-Stage Two-Phase). Модель использует двухэтапную структуру внимания: на первом этапе производится сильный, но децентрализованный отклик весов внимания, а на втором этапе формируется стационарный и сконцентрированный отклик, что позволяет более точно моделировать пространственные корреляции между экзогенными рядами и целевым рядом.

В финансовом контексте DSTP-RNN применяется для долгосрочного прогнозирования многомерных временных рядов, где необходимо учитывать взаимосвязи между различными финансовыми инструментами, индикаторами и макроэкономическими факторами. Модель использует множественные механизмы внимания на целевом ряде для усиления долгосрочных зависимостей, а также глубокий механизм пространственного внимания для извлечения сложных пространственных паттернов. Экспериментальные результаты @liu2019dstp показывают, что attention-based RNN модели превосходят базовые методы на наборах данных в области финансов, энергии, окружающей среды и медицины, демонстрируя эффективность механизма внимания для долгосрочного прогнозирования многомерных временных рядов.
...
*Применение Transformer в прогнозировании финансовых временных рядов.* В работе @cnntransformer2024 исследуется применение архитектур Transformer для прогнозирования финансовых временных рядов. Механизм самовнимания в Transformer позволяет модели явно определять, какие исторические моменты времени наиболее релевантны для прогнозирования текущего значения, что особенно важно в финансовых данных, где различные временные периоды могут иметь различную значимость в зависимости от рыночных условий. Transformer-модели способны параллельно обрабатывать всю последовательность, что ускоряет обучение по сравнению с рекуррентными сетями, и могут моделировать долгосрочные зависимости без ограничений, связанных с затуханием градиента.
...
минимум по $w$ величины $w^T Sigma w$ при условиях $mu^T w >= r_(t a r g e t)$ и $sum_(i=1)^n w_i = 1$,
