...
#figure(
  table(
    ...
    stroke: none,
    table.hline(stroke: 1pt + black),
    [*Модель*], [*MAE*], [*RMSE*], [*MSE*], [*IC ↑*], [*Dir. Acc.*],
    table.hline(stroke: 0.6pt + luma(120)),
    [LSTM],        [0.01350], [0.02159], [0.000466], [*0.0435*], [0.4918],
    [GRU],         [0.01352], [0.02154], [0.000464], [*0.0644*], [0.4885],
    [CNN],         [0.01333], [0.02149], [0.000462], [−0.0043],  [0.4652],
    [Transformer], [0.01333], [0.02149], [0.000462], [0.0109],   [0.4751],
    [LightGBM],    [0.01333], [0.02149], [0.000462], [0.0172],   [0.4775],
    table.hline(stroke: 1pt + black),
  ),
  caption: [Метрики качества моделей на тестовой выборке],
)

Все моделли демонстрируют близкие значения MAE, RMSE и MSE, что ожидаемо для задачи прогнозирования финансовых рядов, где уровень шума высок, а предсказуемая компонента мала.

*R^2* у всех моделей отрицательны или близки к нулю, что характерно для финансовых временных рядов: модели не объясняют дисперсию доходностей в абсолютном смысле, однако сохраняют относительную предсказательную способность, достаточную для построения торговой стратегии.

Так что отдельное внимание уделяется *IC* (Information Coefficient) — ранговая корреляция предсказаний и реальных доходностей. И *Dir. Accuracy* (доля правильно угаданных направлений движения).

*LSTM* демонстрирует второй по качеству результат по метрике *IC* = 0.044, но превосходит оставльные модели по *Dir. Accuracy* = 0.4918.

Это может означать, что модель плохо предсказывоет мелкие изменения цены, но хорошо улавливает крупные, что намного важнее для торговых стратегий.
...
Все остальные модели имеют значения *IC* близкие к 0.1, кроме *ARIMA* и *CNN*, которые имеют малую отрицательную корреляцию с реальными доходностями.
...
#figure(
  ...
  caption: [Метрики — Handmade C3: кластерные vs baseline (жирным — кластерная лучше baseline)],
)
