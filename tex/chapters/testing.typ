...
= Методология тестирования <sec-testing>

== Общая схема эксперимента

Вся экспериментальная часть построена по единой последовательной схеме, включающей пять этапов:
+ Подготовка данных
...
Оптимизируемая метрика — информационный коэффициент (IC) на валидационной выборке @kan2007portfolio.
...
Транзакционные издержки составляют 0.015% от суммы сделки, а безрисковая ставка принимается равной 16% годовых (ключевая ставка ЦБ РФ на период тестирования), пересчитанной в дневной эквивалент.



Для каждой модели и каждой акции вычисляются торговые метрики (включая коэффициент Шарпа @sharpe1994sharpe и коэффициент Калмара @young1991calmar):
